...
---------------------------------------------------*/	
	...
	function CleanupBeforeAjax(){		
		...
		KenndeclouvSlider.instances.forEach(slider => slider.off());
		KenndeclouvSlider.instances = [];
	}
		...
		function initOnFirstLoad() {
			...
			imagesLoaded('body', function() {
				...
				setTimeout(function(){
					...
					gsap.set($(".showcase-gallery .kenndeclouv-slider .slide-inner"), { opacity: 0 });
					gsap.to($(".showcase-gallery .kenndeclouv-slider .kenndeclouv-slide .slide-inner"), { duration: 2, opacity: 1, delay: 0.3, ease: Power4.easeOut });
					
					var gallerySlideClasses = [".kenndeclouv-slide-prev-two", ".kenndeclouv-slide-prev", ".kenndeclouv-slide-active", ".kenndeclouv-slide-next", ".kenndeclouv-slide-next-two"];
					
					gallerySlideClasses.forEach(function(gallerySlideClass, index) {
						var gallerySlide = $(".showcase-gallery .kenndeclouv-slider " + gallerySlideClass + " .slide-inner");
					...
					gsap.set($("#filters-wrapper, .kenndeclouv-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"), {opacity:0});
					gsap.to($("#filters-wrapper, .kenndeclouv-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"), {duration: 0.4, opacity:1, delay:0.8, ease:Power3.easeOut});
...
---------------------------------------------------*/	
	...
	function ShowcasePortfolio() {
		...
		if( $('.showcase-portfolio').length > 0 ){
			...
			function filter() {
				var state = Flip.getState('.kenndeclouv-item');
				var projects = document.querySelectorAll('.kenndeclouv-item');
				var startHeight = gsap.getProperty(".showcase-portfolio", "height");
				
				var filters = document.querySelectorAll('.filter-option.is_active');
				var parallaxItems = document.querySelectorAll('.showcase-portfolio .kenndeclouv-item.vertical-parallax .slide-inner');
			...
			if (!isMobile()) {	
				...
				$(".showcase-portfolio .kenndeclouv-item .slide-inner").on('mouseenter', function() {
					$('#ball p').remove();
					var $this = $(this);			
					gsap.to('#ball', {duration: 0.3, borderWidth: '2px', scale: 1.4, borderColor:"rgba(255,255,255,0)", backgroundColor:"rgba(255,255,255,0.1)"});
					gsap.to('#ball-loader', {duration: 0.2, borderWidth: '2px', top: 2, left: 2});
...
---------------------------------------------------*/	
	...
	function ShowcaseGallery() {
		...
		if( $('.showcase-gallery').length > 0 ){
			...
			slider = new KenndeclouvSlider('.showcase-gallery', { 
				...
				on: {	
					init : function(slide) {
						...
						if ($('body').hasClass("show-loader")) {
							...
							imagesLoaded('body', function() {
								...
								gsap.to($(".showcase-gallery .kenndeclouv-slider .kenndeclouv-slide .slide-inner"), { duration: 0.7, opacity: 1, delay: 0.4, ease: Power2.easeOut });
								...
								gsap.to($("#filters-wrapper, .kenndeclouv-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"), {duration: 0.5, opacity:1, delay:0.4, ease:Power2.easeOut});
					...
					slideLeaveViewport : function( slide ) {						
						gsap.set($('.kenndeclouv-slider div:not(.kenndeclouv-slide-visible) .slide-events'), { x: "" });						
					},
			...
			const showGrid = () => {
				...
				gsap.to($("footer .link-text, .kenndeclouv-pagination, .progress-info, #filters-wrapper"), {duration: 0.3, opacity:0, y:30, stagger:-0.05, ease:Power2.easeInOut});
				...
				gsap.set(currentImage.closest(".kenndeclouv-slide"), {zIndex: 0});
				...
				gsap.to(".kenndeclouv-slider .kenndeclouv-slide .trigger-item", { duration: 1, opacity:0, scale:0.7, ease:Power2.easeOut });
			...
			const hideGrid = () => {
				...
				gsap.to(".kenndeclouv-slider .kenndeclouv-slide .trigger-item", { duration: 0.5, opacity:1, scale:1, delay:0.2, ease: 'power3.inOut' });				
				...
				Flip.from(flipstate1, {
					...
					onComplete: function() {
						...
						const triggeredItem = document.querySelector('.kenndeclouv-slide.triggered-item');
						if( triggeredItem != null ){							
							triggeredItem.classList.remove('triggered-item');
						}
						
						const Slides = document.querySelectorAll('.kenndeclouv-slide');
				...
				gsap.to($("footer .link-text, .kenndeclouv-pagination, .progress-info, #filters-wrapper"), {duration: 0.3, opacity:1, y:0, stagger:0.05, delay:0.4, ease:Power2.easeInOut});
				gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '4px', top: 0, left: 0});
				$("#ball").removeClass("with-blur");
				$('#ball p').remove();
			...
			const getDOMElements = () => {
				...
				const items = [];
				[...document.querySelectorAll('.kenndeclouv-slide')].forEach(item => {
					items.push(new Item(item));
				});
						
				// Cloned items
				const itemsCloned = [];
				[...document.querySelectorAll('.kenndeclouv-slide-clone')].forEach(item => {
				...
				return {					
					items: items,					
					grid: gridItems,					
					currentItem: new Item( document.querySelector('.kenndeclouv-slide.triggered-item') ),
			...
			const initEvents = () => {
				...
				items.forEach((triggerItem) => {
					...
					triggerItem.addEventListener('click', (event) => {
						...
						if( $('.showcase-gallery').length > 0 ){
							...
							event.currentTarget.closest('.kenndeclouv-slide').classList.add('triggered-item');
							showGrid();
						}
					});
				...
				function moveThumbGrid( direction = "next" ){
					...
					if(document.body.classList.contains("grid-open")) {
						...
						// Get all the slides
						let slides = document.querySelectorAll('.kenndeclouv-slide');
						...
						$('.kenndeclouv-slide').removeClass('triggered-item');
			...
			if (!isMobile()) {
				...
				if ($('.showcase-gallery').hasClass("tilt-gallery")) {
					...
					function callTiltSlider(e) {
						moveItSlider(e, '.kenndeclouv-slider-viewport', 30);
					}
				...
				var dragWrapper = $('.kenndeclouv-slider');
				...
				$('.kenndeclouv-slider').on('mouseup touchend', function() {
				...
				$(".showcase-gallery.preview-mode-enabled .kenndeclouv-slide .slide-inner").on('mouseenter', function() {	
